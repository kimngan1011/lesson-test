...
export class LsPopup {
  ...
  public async getNextLessonDate(fieldName: string) {
    ...
    const nextLessonDate = currentDate.toLocaleDateString(
      "en-GB",
      formatOptions
    );
    await this.page.getByLabel(fieldName, { exact: true }).click();
  ...
  public async searchAndSelectData(fieldName: string, optionName: string) {
    ...
    await this.page
      .getByPlaceholder(fieldName, { exact: true })
      .fill(optionName);
    await this.page.getByLabel(optionName).locator("span").nth(1).click();
  }

  public async getRecurringSetting(
    recurringType: "endDate" | "courseSchedule"
  ) {
    await this.page
      .locator("lightning-primitive-input-checkbox span")
      .nth(1)
      .click();
    if (recurringType == "endDate") {
      ...
      await this.page
        .locator("label")
        .filter({ hasText: "Course Schedule" })
        .locator("span")
        .first()
        .click();
    }
...
export class StudentPopup {
  ...
  public async inputEmail(value: string) {
    await this.page
      .getByLabel("New Contact: Student")
      .getByLabel("Email")
      .click();
    await this.page
      .getByLabel("New Contact: Student")
